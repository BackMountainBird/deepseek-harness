...
The default artifact is a standard concatenation of independent [Zstandard frames](../../../.agents/notes/implemented/architecture/2026-07-19-zstandard-jsonl-session-logs.md): one checksummed frame containing only the header line, followed by one checksummed frame per durable append batch. The backend uses Node's built-in Zstandard API with its default compression level and exposes no level knob. Listing reads and validates only the header frame: it probes each session directory with two stats and bounded parallelism, and memoizes the header line per file identity (device and inode) — the first frame is immutable under append-only writes, so a warm listing performs no log read, while a replaced file at the same path re-reads once. `compression: 'none'` keeps the same logical lines in the original raw representation. The [listing memoization decision](../../../.agents/notes/implemented/architecture/2026-08-20-jsonl-list-artifacts-memoization.md) records the invariants.
